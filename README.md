...
| 플러그인 | 하는 일 | 순서 |
|---|---|---|
| **dev** | 개발 워크플로우 — ⓪설계 → ①상세설계 → ②구현 → ③검증 → ④릴리스 | **구현한 것을 검증** |
| **atdd** | 자동 인수 테스트 — 요구 정돈 → 인수 기준 → 인수 테스트 프로그램 | **검증할 것을 먼저 만듦** |

순서가 반대입니다. `dev`는 구현 뒤에 검증하고, `atdd`는 사람이 확정한 인수 기준과 그것을 확인할 테스트를 먼저 만든 뒤 개발로 넘깁니다. 어휘(`docs/glossary.md`)와 harness 설계 원리(`docs/harness-design.md`)만 공유합니다.
...
자동 인수 테스트 플러그인입니다. **요구사항을 정돈해 사람이 인수 기준을 확정하고, 그것을 확인할 층(동작 사전·테스트·경로 실행기)을 프로젝트 안에 만드는 것까지**를 담당합니다. 개발은 그 다음입니다.

**플러그인의 범위는 층 하나입니다** — 요구 정돈 → 인수 기준 → 동작 사전·테스트·경로 실행기. 그것을 돌리는 오케스트레이터와 제품을 띄우는 격리 인프라는 **프로젝트가 세웁니다.** SoT는 `docs/acceptance-test-system/atdd.html`입니다 — 구조·정책·플러그인의 역할·워크플로우가 전부 여기 있습니다.

#### 층 틀 — 플러그인이 갖는 규칙

| 틀 | 정의하는 것 | 축 |
|---|---|---|
| `atdd:frame-layers` | 테스트 하나의 세로 구조 — ①테스트 ②동작 사전 ③경로 실행기 ④제품(인수 기준은 층 밖). 소비·위임·접근, 절대 규칙 둘, **무엇이 문이고 무엇이 아닌가**(DB는 문이 아니다), 문 고르는 규칙, 시나리오+규칙 표기, **별칭**, **검증 대상이 여럿일 때** | 1 · 2 · 3 (+4 일부) |

층의 규칙은 이 틀에만 둡니다. 작성자·단계에 복사하지 않습니다 — 축이 바뀌면 고칠 곳이 한 군데입니다.

**기반은 플러그인이 만들지 않습니다.** 오케스트레이터와 격리 인프라는 언어·도구·배포가 프로젝트마다 달라 규칙으로 강제하지 않습니다. **무엇이 필요한지는 `docs/acceptance-test-system/atdd.html`에 있고, 어떻게 세울지는 프로젝트가 정합니다.**

#### 단계 — 그 틀을 채우는 순서

| 단계 | 하는 일 | 산출물 |
|---|---|---|
| `atdd:requirements` | 요구 정돈 — 난잡하게 들어온 요구에서 검증 행위·갈래·전제·관찰 대상을 꺼낸다. **보조이지 대행 아님** | `acceptance/requirements.md` (사람 확정) |
| `atdd:acceptance` | 인수 기준 작성 — 행위 단위로 분해, 시나리오·규칙을 성격에 따라 골라 표기. 초안은 플러그인, **확정은 사람** | `acceptance/criteria.md` (사람 확정) |
| `atdd:tests` | 테스트 만들기 (**인수 기준마다**) — 동작 사전 → 테스트 → 경로 실행기 | 코드 |
| `atdd:run` | 전체를 순차로 | — |

사람이 확정하는 두 지점은 문서 머리의 확정 표시(`> 확정: <사람>·<날짜>`)로 확인하며, 에이전트가 대신 찍지 않습니다.

**전제** — 프로젝트에 기반(격리 인프라 · 오케스트레이터 · 층을 둘 자리)이 이미 서 있어야 합니다. 없으면 `atdd:tests`가 멈추고 사람에게 알립니다.
...
| 종류 | 이름 | 역할 |
|---|---|---|
| Agent | `requirements-facilitator` | 정돈된 요구사항 (사람 확정 보조) |
| Agent | `acceptance-writer` | 인수 기준 텍스트 — 원본, 기획자 사후 검수 대상 |
| Agent | `dictionary-writer` | 동작 사전 — 전제 준비 / 검증 행위 / 확인. **별칭을 갖는다**(회차 표 + 번호) |
| Agent | `driver-writer` | 경로 실행기 — 경로마다 하나. **제품에 손대는 유일한 층** |
| Agent | `test-writer` | 테스트 코드 — 확정된 인수 기준을 **옮기기만** 하고 재해석하지 않음 |

#### 끝점과 그 이후

파이프라인이 끝나면 프로젝트 안에 인수 테스트 프로그램이 서 있고 돌 수 있습니다. **제품이 아직 없으므로 돌리면 테스트가 실패하며, 그것이 정상 종료 상태입니다.**

개발이 끝난 뒤 테스트를 돌리는 것은 **플러그인이 아니라 프로젝트 안의 오케스트레이터**입니다. 에이전트 없이 도는 스크립트라 CI에 그냥 걸립니다. 플러그인으로 돌아오는 것은 인수 기준을 고치거나 테스트를 늘릴 때뿐입니다.

#### 범위 밖 (설계 SoT가 그렇게 정함)

게이트(미통과 시 차단) 설계 · 병렬 실행 · 제품 구현. 축4 숫자(수용소 상한·골든셋 크기·통과율 임계값)와 이력 저장소 위치는 프로젝트마다 정합니다.
