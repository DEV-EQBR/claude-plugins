...
| 분석 근거 | 포함된 각 fragment의 "갱신된 명세서"·"코드 변경"·"마이그레이션" 섹션과, 필요 시 갱신된 spec/ 명세서 |
| bump 분류 — major | 하위 호환을 깨는 변경. 공개 API/계약 제거·시그니처 변경, 비호환 스키마 마이그레이션, 기존 동작의 호환성 파괴, 명세서상 breaking 표기 |
| bump 분류 — minor | 하위 호환을 유지하는 기능 추가. 신규 endpoint·필드·도메인·시나리오 추가, 기존 계약을 깨지 않는 확장 |
| bump 분류 — patch | 동작 추가/제거 없는 수정. 버그 픽스, 내부 리팩터, 문서/명세 표현 정정 |
...
2. 포함된 각 fragment의 변경 내용을 분석한다 — 갱신된 명세서/코드 변경/마이그레이션을 읽고 변경 종류를 식별한다
3. 판별이 모호한 fragment는 갱신된 spec/ 명세서로 변경의 호환성 영향을 확인한다
