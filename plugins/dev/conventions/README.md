...
명세서 작성을 가이드하는 별도 문서 모음. 명세서 본문이 담는 정보(시스템/서비스/도메인)와는 별개로, 그 본문을 작성할 때 검토할 작성 기준이다.

## 정체성

| 항목 | 내용 |
|------|------|
| 정의 | 명세서 작성 시 검토하는 작성 기준 문서 모음 |
| 본문에 담는 것 | 메타-규칙(원칙) + 결정 항목(spec에 박힐 답을 가진 질문) + spec 작성 예 |
| 본문에 담지 않는 것 | 구체 결정값 (그 결정은 각 프로젝트의 명세서에 인라인으로 박힘) |
| 변경 빈도 | 낮음. 팀 정책에 가까움 |
| 위치 | `plugins/dev/conventions/` |

명세서가 단일 진실 출처(SSOT)라는 원칙은 변하지 않는다. 컨벤션 본문은 명세서 작성을 도울 뿐, 구현 단계의 참조 대상이 아니다. 결정의 결과는 명세서에 인라인으로 박힌다.
...
분리 축은 명세서의 정보 분류(시스템/서비스)를 그대로 따른다. 어느 명세서에 결과가 박힐지가 디렉토리 구조만 봐도 명확하다.
...
| 결정 항목 | 이 컨벤션을 따르려면 결정하여 명세서에 명시해야 할 항목 표 (결정 항목 + 명시 위치 + 형태/제약) |
| spec 작성 예 | 결정의 결과가 명세서에 어떻게 박히는지 샘플 |
...
명세서 작성 에이전트(`system-service-writer`)가 자기 절차에서 카탈로그를 자동 검토한다.

1. 카탈로그 전체 훑기 (`system/*.md`, `service/{레이어}/*.md`)
2. 각 컨벤션의 "적용 조건"과 설계서/기존 명세서를 대조하여 적용 여부 판단
3. 적용된 컨벤션의 "결정 항목"을 질문 목록에 포함하여 개발자에게 묻기
4. 답변을 받으면 "spec 작성 예" 형식을 참조하여 명세서에 인라인으로 박기
...
- 명세서 작성 중 "이 결정은 컨벤션화할 만한데 카탈로그에 없음"을 발견
- 팀 표준 정립 회의에서 결정
...
- spec에 박힐 결정값을 본문에 흡수하지 않는다 (그 결정은 프로젝트 spec에 박힘)
