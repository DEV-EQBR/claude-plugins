...
요청·응답 본문 필드는 프로젝트 전역에서 **단일 케이싱 정책**을 따른다. timestamp, actor, ID, boolean 등 도메인이 반복적으로 사용하는 필드는 표준 명명 규칙을 두어 도메인마다 다른 이름을 짓지 않게 한다. 도메인 api.md의 필드명은 entities.md의 속성 명명과 일치해야 한다 — 둘이 어긋나면 spec-verifier가 위반으로 검출한다.
