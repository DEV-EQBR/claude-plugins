...
| 성공 응답 구조 | `service/{백엔드 서비스}.md` / 응답 래퍼 섹션 | `success` 방식 선택 시: `{ success: true, data: ..., message?: string }` 등 정확한 키 상세설계. `data` 필드 사용 여부와 옵셔널 키도 결정 |
| 실패 응답 구조 | `service/{백엔드 서비스}.md` / 응답 래퍼 섹션 | `{ success: false, error: { code, message }, details?: ... }` 형태로 키 상세설계. `error` 객체의 내부 키와 추가 필드(`details`, `traceId` 등) 사용 조건 명시 |
