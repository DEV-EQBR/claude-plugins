...
오케스트레이터의 **작업 상태**(plan·진행·결정 로그·보유 버전)를 컨텍스트가 아니라 파일에 두어, 컴팩션·세션 종료를 넘어 **이어서 진행**할 수 있게 한다. 각 단계 스킬(`dev:run`·`design`·`spec`·`implement`·`verify`·`release`)이 이 규약을 따른다.

> 설계 배경: 산출물(spec/·fragment·spec-archive)은 외부화돼 있었으나 오케 *작업 상태*는 컨텍스트에만 있었다. run-state는 그 빈자리를 메운다. (`docs/harness-design.md` §6)

## 1. 위치 · 키 · 수명

- 위치: **`.dev/<스프린트 버전>/`** (워크트리당 1개). 키 = `<forked-from>-<워크트리키>` — fragment·spec-archive와 동일 좌표(시작 시점에 결정론적으로 알려짐).
...
| 담는 것 | 오케 단계/단위 진행 | 떠 있는 인프라·서비스·포트·검증DB·health |
...
- 실행 shape: 병렬 (order·payment 독립) | 중첩 | 순차
...
`pipeline.md`(dev:run)는 `## 계획` 대신 `## 단계 진행`(design·spec·implement·verify·release 각각 start/done) + `> 보유 vX.Y.Z` + `## cross-stage 결정`.

## 4. 쓰기 규율 — start/done 마커

신뢰성은 런타임이 아니라 **이 규율**이 보장한다.

- 어떤 단위/하위 작업을 **위임하기 직전**: `시작 ✓` 마커를 쓴다.
- 그 작업이 **완료·확인된 직후**: `끝 ✓` 마커를 쓴다.
- 단계 경계(단계 전체 완료): `dev:run`이 `pipeline.md`의 단계 진행을 전진시킨다.
- escalation 발생 시 질문을 즉시, 답변 수령 시 즉시 로그에 쓴다.

→ "시작 있고 끝 없음" = 그 작업이 멈춘 지점. 작고 원자적으로, 다음 위임 전에 쓴다.
...
2. **start/done 마커로 멈춘 지점을 판정**:
   ...
   - `시작 ✓` 있고 `끝` 없음 → 그 항목을 **재실행**(재위임). 재위임 handoff 팩 = escalation 로그의 답변 + 파일 경로(에이전트가 spec/·코드에서 맥락 회복).
...
- **내용의 권위는 아티팩트**(spec/·fragment·코드·git·tag)다. run-state는 내용을 담지 않는다.
- 따라서 **결정 *과정*은 run-state(escalation 로그), 결정된 *값*은 fragment**(`## 가정/결정`)에 — `dev:spec`이 outcome을 fragment로 옮긴다.
