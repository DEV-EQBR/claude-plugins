...
명세서 한 파일이 다른 파일의 SoT를 참조할 때, 그 참조가 실제로 존재하는지 검사한다.
...
- fragment_lists_updated_files   (fragment '갱신된 명세서' 파일 경로 존재)
- fragment_file_version_matches  (fragment '파일 버전' ↔ spec 파일 메타데이터 '버전' 일치)
- fragment_archive_exists        (spec-archive/<스프린트 버전>/{spec이하경로} 존재 + 메타 '버전' 일치)
- release_archive_exists         (CHANGELOG.md fold 엔트리 ## v{X.Y.Z} ↔ spec-archive/v{X.Y.Z}/ 존재)
"""
...
def index_existing_domains(project_root: Path) -> set[str]:
    """spec/domain/ 하위 도메인 디렉토리명 집합 (cross-domain 제외)."""
...
def extract_fragment_updated_specs(content: str) -> list[ChangelogRow]:
    """fragment(changelog.d/*.md)의 '## 갱신된 명세서' 표를 파싱.
    ...
    for i, raw in enumerate(content.splitlines(), start=1):
        ...
        if head:
            in_section = (len(head.group(1)) == 2
                          and head.group(2).strip() == "갱신된 명세서")
            in_table = False
...
def run(catalog: dict, project_root: Path) -> CheckResult:
    ...
    for rf in expand_file_kind("cross_domain_flow", fk["cross_domain_flow"], project_root):
        for tag, line_no in extract_cross_domain_tags(read_text(rf.path)):
            ...
            result.add(Violation(
                ...
                message=f"흐름이 참조한 도메인 [{tag}]에 해당하는 spec/domain/{tag}/ 디렉토리가 없다",
            ))

    # fragment(changelog.d/*.md)의 '갱신된 명세서' 표 검증
        ...
        for rf in expand_file_kind("fragment", fk["fragment"], project_root):
            ...
            for row in extract_fragment_updated_specs(content):
                ...
                if row.software_version is None:
                    ...
                    continue
                if not row.path.startswith("spec/"):
                    # spec/ 외 파일은 archive 매핑이 없으므로 스킵 (현재 정책상 발생하지 않아야 함)
                    continue
                rel_under_spec = row.path[len("spec/"):]
                archive_path = (
                    project_root / "spec-archive" / row.software_version / rel_under_spec
                )
                if not archive_path.is_file():
                    result.add(Violation(
                        ...
                        message=(
                            f"spec-archive 스냅샷 '{archive_path.relative_to(project_root)}' "
    ...
    # release(CHANGELOG.md) fold 엔트리 ↔ 최종 spec-archive 디렉토리 검증.
    ...
    if "release_changelog" in fk and release_cfg:
        ...
        for rf in expand_file_kind(
            ...
            for version, line_no in extract_release_versions(
                ...
                # spec-archive/v{X.Y.Z}/ 또는 v 접두사 없는 {X.Y.Z}/ 둘 다 인정.
                # (작성 가이드가 v 접두사를 강제하지 않으므로 — 메타 원칙)
                candidates = [
                    project_root / "spec-archive" / f"v{version}",
                    project_root / "spec-archive" / version,
                ]
                if not any(d.is_dir() for d in candidates):
                    result.add(Violation(
                        ...
                        message=(
                            f"릴리스 엔트리 'v{version}'에 대응하는 spec-archive 스냅샷 "
                            f"'spec-archive/v{version}/'이 존재하지 않는다 "
                            f"(릴리스 fold 시 최종 명세서 스냅샷 누락)"
                        ),
                    ))
