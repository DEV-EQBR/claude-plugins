...
_R_RE = re.compile(r"\bR-\d+\b")
_ACCEPTANCE_FACETS = ["UI/UX", "로직", "데이터"]
...
def _check_acceptance_table(content: str, path: Path, result: CheckResult) -> None:
    """채택(requirements.md 존재) 시 `## 완료 기준` 표를 검증한다.

    각 데이터 행: AC 칸에 AC-n, 요구사항 칸에 R-n(≥1), 3면(UI/UX·로직·데이터) 셀
    non-empty(빈 셀이면 관찰 가능한 완료 기준 미분해). AC·요구사항 칼럼 부재는
    채택 위반 — 이 부분은 check_traceability가 자세히 잡으므로 여기선 3면 셀·행별
    ID 존재에 집중(작성 시점 예방).
    """
    body = _section_body(content, "완료 기준")
    if not body:
        result.add(Violation(
            file=str(path), file_kind=DESIGN_KIND, rule="acceptance_missing",
            message="requirements.md 채택 상태인데 '## 완료 기준' 섹션이 없다.",
        ))
        return
    ...
    for raw in body:
        ...
        if header is None:
            header = cells
            continue
        if not seen_sep or not any(cells):
            continue
        # 데이터 행 검증
        def col(name):
            return cells[header.index(name)] if name in header and header.index(name) < len(cells) else ""
        row_label = (col("AC") or col("요구사항") or (cells[0] if cells else "?"))[:24]
        if "AC" in header and not _AC_RE.search(col("AC")):
            result.add(Violation(
                file=str(path), file_kind=DESIGN_KIND, rule="acceptance_ac_id",
                message=f"완료 기준 행 '{row_label}…'의 AC 칸에 AC-n ID가 없다.",
            ))
        if "요구사항" in header and not _R_RE.search(col("요구사항")):
            result.add(Violation(
                file=str(path), file_kind=DESIGN_KIND, rule="acceptance_req_ref",
                message=f"완료 기준 행 '{row_label}…'의 요구사항 칸에 R-n 참조가 없다.",
            ))
        for facet in _ACCEPTANCE_FACETS:
            if facet in header and not col(facet):
                result.add(Violation(
                    file=str(path), file_kind=DESIGN_KIND, rule="acceptance_facet_empty",
                    message=(
                        f"완료 기준 행 '{row_label}…'의 '{facet}' 면 셀이 비어 있다 "
                        "— 각 완료 기준은 3면(UI/UX·로직·데이터)으로 관찰 가능하게 분해돼야 한다."
                    ),
                ))
...
def run(catalog: dict, project_root: Path) -> CheckResult:
    ...
    # 6. 완료 기준 표 (requirements.md 채택 시): AC-n·R-n·3면 셀
    req_rel = ((catalog.get("file_kinds") or {}).get("requirements") or {}).get(
        "path", "design/requirements.md")
    if (project_root / req_rel).is_file():
        _check_acceptance_table(content, path, result)
